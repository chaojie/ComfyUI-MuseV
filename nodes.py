...
from MuseVdiffusers.models.autoencoder_kl import AutoencoderKL

from .MMCM.mmcm.utils.load_util import load_pyhon_obj
from .MMCM.mmcm.utils.seed_util import set_all_seed
from .MMCM.mmcm.utils.signature import get_signature_of_string
from .MMCM.mmcm.utils.task_util import fiss_tasks, generate_tasks as generate_tasks_from_table
from .MMCM.mmcm.vision.utils.data_type_util import is_video, is_image, read_image_as_5d
from .MMCM.mmcm.utils.str_util import clean_str_for_save
from .MMCM.mmcm.vision.data.video_dataset import DecordVideoDataset
...
def parse_args():
    ...
    parser.add_argument(
        "-test_data_path",
        type=str,
        default=os.path.join(PROJECT_DIR, "configs/tasks/example.yaml"),
        help=(
    ...
    parser.add_argument(
        ...
        type=str,
        default=f"{MuseVCheckPointDir}/ip_adapter/models/image_encoder",
    ...
    parser.add_argument(
        "--vae_model_path",
        default=f"{MuseVCheckPointDir}/vae/sd-vae-ft-mse",
        type=str,
...
class MuseVRun:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "ref_image": ("IMAGE",),
                "prompt":("STRING",{"default":"(masterpiece, best quality, highres:1),(1girl, solo:1),(beautiful face, soft skin, costume:1),(eye blinks:1.8),(head wave:1.3)"}),
                "img_edge_ratio":("FLOAT",{"default":1.0}),
                "video_len":("INT",{"default":12}),
                "seed":("INT",{"default":1234}),
                "width":("INT",{"default":563}),
                "height":("INT",{"default":714}),
                "first_redraw_steps":("INT",{"default":30}),
                "video_steps":("INT",{"default":10}),
                "first_redraw_guidance_scale":("FLOAT",{"default":7.5}),
                "video_guidance_scale":("FLOAT",{"default":3.5}),
                "video_steps":("INT",{"default":10}),
                "sd_model_name":(["majicmixRealv6Fp16","fantasticmix_v10"],{"default":"majicmixRealv6Fp16"}),
                "uselcm":("BOOLEAN",{"default":False}),
            },
        }
    ...
    def run(self, ref_image,prompt,img_edge_ratio,video_len,seed,width,height,first_redraw_steps,video_steps,first_redraw_guidance_scale,video_guidance_scale,sd_model_name,uselcm):
        args = parse_args()
        args.sd_model_name=sd_model_name
        #print("args")
        #print(args)
        #print("\n")
        ...
        video_guidance_scale = video_guidance_scale  # 视频 condition与 uncond的权重参数
        guidance_scale = first_redraw_guidance_scale  # 时序条件帧 condition与uncond的权重参数
        video_num_inference_steps = video_steps  # 视频迭代次数
        num_inference_steps = first_redraw_steps  # 时序条件帧 重绘参数
        #seed = args.seed
        ...
        b = 1
        negative_embedding = [
            [f"{MuseVCheckPointDir}/embedding/badhandv4.pt", "badhandv4"],
            [
                f"{MuseVCheckPointDir}/embedding/ng_deepnegative_v1_75t.pt",
                "ng_deepnegative_v1_75t",
            ],
            [
                f"{MuseVCheckPointDir}/embedding/EasyNegativeV2.safetensors",
                "EasyNegativeV2",
            ],
            [
                f"{MuseVCheckPointDir}/embedding/bad_prompt_version2-neg.pt",
        ...
        image_path = os.path.join(PROJECT_DIR, f"data/images/{image_cuid}.jpg")
...
class MuseVRunVid2Vid:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "ref_image": ("IMAGE",),
                "prompt":("STRING",{"default":"(masterpiece, best quality, highres:1),(1girl, solo:1),(beautiful face, soft skin, costume:1),(eye blinks:1.8),(head wave:1.3)"}),
                "video_path": ("STRING",{"default":PROJECT_DIR+"/data/source_video/video1_girl_poseseq.mp4 "}),
                "img_edge_ratio":("FLOAT",{"default":1.0}),
                "video_len":("INT",{"default":12}),
                "seed":("INT",{"default":1234}),
                "width":("INT",{"default":563}),
    ...
    def run(self, ref_image,prompt,video_path,img_edge_ratio,video_len,seed,width,height,first_redraw_steps,video_steps,first_redraw_guidance_scale,video_guidance_scale,sd_model_name,uselcm):
        args = parse_args()
        args.sd_model_name=sd_model_name
        #print("args")
        #print(args)
...
NODE_CLASS_MAPPINGS = {
    "MuseVRun":MuseVRun,
    "MuseVRunVid2Vid":MuseVRunVid2Vid,
    "VHS_FILENAMES_STRING_MuseV":VHS_FILENAMES_STRING_MuseV,
}